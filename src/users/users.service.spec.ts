...
describe('UsersService', () => {
  ...
  describe('create', () => {
    it('debe hashear password, crear y guardar usuario', async () => {
      ...
      expect(repo.create).toHaveBeenCalledWith({ ...dto, password: 'hashed-10' });
    ...
    it('debe retornar null si ocurre error', async () => {
      ...
      const result = await service.create({ username: 'x', email: 'x@x.com', password: '123' } as any);
  ...
  describe('findAll', () => {
    it('debe aplicar isActive, searchField=username, sort y llamar paginate', async () => {
      ...
      expect(qb.andWhere).toHaveBeenCalledWith('user.username ILIKE :search', { search: '%jo%' });
      expect(qb.orderBy).toHaveBeenCalledWith('user.username', 'DESC');
      expect(paginate).toHaveBeenCalledWith(qb as unknown as SelectQueryBuilder<User>, { page: 1, limit: 10 });
